...
interface FeeCollectionSectionProps {
  ...
  // CURRENT-year outside-org fee editor (display-only context) — lives here,
  // beside the team fee, because this is where the coach decides THIS year's
  // fee and needs the total family burden in view. The draft planner stages
  // next year's value separately (nextExternalOrgFee).
  orgFeeLabelInput: string | null;
  setOrgFeeLabelInput: Dispatch<SetStateAction<string | null>>;
  orgFeeAmountInput: string | null;
  setOrgFeeAmountInput: Dispatch<SetStateAction<string | null>>;
  commitExternalOrgFee: () => void;
...
export const FeeCollectionSection = ({
  ...
  setOrgFeeLabelInput,
  orgFeeAmountInput,
  setOrgFeeAmountInput,
  commitExternalOrgFee,
  setFinanceFields,
  toast,
  ...
  // Outside-org fee context (display only, e.g. the parent org's fee families
  // pay directly). Shown beside the team fee — the moment the coach decides
  // it — so the TOTAL family burden is in view. Never enters any money math.
  ...
  return (
    ...
    >
      <div className="py-3 border-b border-line space-y-2">
        ...
        {/* CURRENT-year outside-org fee editor — reachable right where the
            coach sets this year's fee, not only in the draft planner. Writes
            the display-only externalOrgFee context; never any money math. */}
        <OrgFeeEditor
          eyebrow="Outside org fee"
          ariaPrefix="Outside org"
          stored={finances.externalOrgFee}
          labelInput={orgFeeLabelInput}
          setLabelInput={setOrgFeeLabelInput}
          amountInput={orgFeeAmountInput}
          setAmountInput={setOrgFeeAmountInput}
          commit={commitExternalOrgFee}
          hint="per player, paid directly by families — context only, never counted"
        />
        {orgFee && (
          <p className="t-meta text-ink-3">
            ...
            . Outside the club&apos;s books — shown for context only.
          </p>
        )}
        {summary.duesCreditPerPlayer > 0 && (
          <p className="t-meta text-ink-3">
            ...
            fees — unless an entry is credited to a specific child, in which
            case it comes off that child&apos;s fees first.
          </p>
        )}
