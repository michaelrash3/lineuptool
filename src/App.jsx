...
/* ============================================================================
   SECTION 3 · Pure helpers (formatting, parsing)
============================================================================ */
...
const normalizeDateToIso = (dateString) => {
  ...
  if (!trimmed) return "";
  // Already ISO (with optional time portion)
  const isoMatch = trimmed.match(/^(\d{4})-(\d{2})-(\d{2})/);
  if (isoMatch) return `${isoMatch[1]}-${isoMatch[2]}-${isoMatch[3]}`;
  // ISO without zero-padding (e.g., 2026-4-27)
  ...
  // US slash format MM/DD/YYYY or M/D/YYYY or M/D/YY
  ...
  // Last-resort fallback: let Date parse it and convert. Beware: this can
  // misread ambiguous dates due to timezones, so use only when nothing else
  // matches.
  const d = new Date(trimmed);
  if (!Number.isNaN(d.getTime())) {
    // Use local-time components to avoid TZ shift
...
const formatGameDateDisplay = (dateString) => {
  if (!dateString) return "";
  const iso = normalizeDateToIso(dateString);
  if (!iso) return dateString; // fallback to raw if completely unparseable
  try {
...
// Compute each player's cumulative season bench-vs-fielding imbalance.
// Returns Map<playerId, { extraSits: number, totalBench: number, totalDefense: number }>.
//   extraSits — how many MORE than the per-game minimum the player has sat
//               across all final games this season. Higher = "owed more
//               playing time" in future games. Negative isn't possible (it's
//               capped at 0 by the per-game floor).
// We use this as a coach-facing visibility signal — the engine has its own
// independent computation in buildExtraSitHistory.
// Computes each player's season-cumulative defensive innings AND their
// expected innings (the fair share they would have earned given the games
...
const buildSeasonBenchImbalance = (games, currentGameId) => {
  ...
  for (const g of games || []) {
    ...
    // What each attending player would earn in defense if shared perfectly:
    // total defense slots / number of attending players
...
const calculateBaseballAge = (dob, currentSeasonStr) => {
  if (!dob) return null;
...
/* ----- CSV header detection (extracted for clarity & testability) ----- */
const buildCsvHeaderIndex = (headers) => {
  const find = (...keys) => {
    for (const k of keys) {
...
// Each row is { name, stats } — caller decides which player each row maps to.
const parseGameChangerPastSeasonCsv = (text) => {
  text = (text || "").replace(/^\uFEFF/, "");
  const lines = text.split(/\r?\n/).filter((l) => l.trim());
  if (lines.length < 2) return { error: "File appears to be empty.", rows: [] };

  // Detect two-row header (GameChanger has section labels on row 0)
  ...
  // Reject TeamSnap files immediately — past-season import is GameChanger only
  if (idx.isTeamSnap) {
    return {
      error:
  ...
  // Sanity: must have at least some hitting fields
  ...
  for (let i = dataStart; i < lines.length; i++) {
    ...
    // Build the stats patch the same way the current-season importer does.
    const stats = {};
...
// Uses a simple normalized comparison + last-name match fallback.
const suggestPlayerMatch = (csvName, players) => {
  ...
  // First name + last initial match (e.g., "Liam S." vs "Liam Smith")
...
// Fall → Spring keeps the same tier (since Fall is "playing up" for next Spring).
const AGE_TIERS = [
  "6U",
  "7U",
  "8U",
...
const computeNextSeason = (currentSeasonStr) => {
  const parts = (currentSeasonStr || "").split(" ");
...
const buildLineupCanvas = ({ game, team, formatDate }) => {
  ...
  // Probe with the cell font, then size each inning column to fit.
  ctx.font = "600 11px system-ui, -apple-system, Segoe UI, sans-serif";
  let maxNameW = 40; // floor
  ...
  // Add cell padding (6px each side)
  const inningColW = Math.ceil(maxNameW + 12);
  const PAD = 24;
  const labelColW = 64;
  // Compute total width: padding + label col + N inning cols + padding
  const minW = 600;
  const maxW = 1100; // hard cap so really long names don't blow up
  ...
  const cellH = 38;
  const defenseH = sectionTitleH + cellH * (presentPositions.length + 1); // +1 for header row
  ...
  // Date + time on the right side of header
  ...
  for (let r = 0; r < presentPositions.length; r++) {
    ...
    // Player full name in each inning
    ctx.fillStyle = "#0f172a"; // slate-900
    ctx.font = "600 11px system-ui, -apple-system, Segoe UI, sans-serif";
    for (let i = 0; i < totalInnings; i++) {
      ...
      // Only truncate if we're over the cap (max width). For normal cases the
      // column was sized to fit the longest name, so this is rarely needed.
...
// user's "Share to Messages" sheet appears), falls back to a download link.
const shareLineupCard = async ({ game, team, formatDate, toast }) => {
  try {
    const blob = await renderLineupCard({ game, team, formatDate });
    if (!blob) throw new Error("Image generation failed");
    const filename = `lineup-${game.opponent || "game"}-${game.date || "card"}.png`.replace(/\s+/g, "-").toLowerCase();
    const file = new File([blob], filename, { type: "image/png" });

    // Try the Web Share API first (preferred on mobile — opens iOS/Android share sheet)
    if (navigator.share && navigator.canShare && navigator.canShare({ files: [file] })) {
      try {
        await navigator.share({
          files: [file],
        ...
        if (e.name === "AbortError") return;
        // Other failure — fall through to download path.
      }
    }
