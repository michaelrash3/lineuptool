...
export const AdvanceSeasonModal = memo(
  ({
    ...
    useEffect(() => {
      if (!open) return;
      const accepted = (tryoutSignups || []).filter(
        (s: any) => s.status === "accepted",
      );
      setPromoteIds(new Set(accepted.map((s: any) => s.id)));
      setDepositPaid(
        Object.fromEntries(
          accepted.map((s: any) => [s.id, s.depositPaid === true]),
        ),
      );
      setDepositPaidAt(
        Object.fromEntries(
    ...
    const setAllSignups = (on: boolean) => {
      setPromoteIds(on ? new Set(sortedSignups.map((s) => s.id)) : new Set());
    };
        ...
        >
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto px-6 sm:px-7 pb-4 space-y-5">
              {togglablePlayers.length === 0 &&
              partition.accepted.length === 0 ? (
