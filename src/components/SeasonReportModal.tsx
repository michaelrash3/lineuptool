...
export const SeasonReportModal = memo(({ open, onClose, team }: any) => {
  ...
  const leaders = useMemo(() => {
    const defs: LeaderDef[] = [
      { label: "Top Hitter (OPS)", kind: "dec3", hi: true, get: (s) => read(s, "ops") },
      { label: "Top Average", kind: "dec3", hi: true, get: (s) => read(s, "avg") },
      ...
      { label: "Iron Glove (FPCT)", kind: "dec3", hi: true, get: (s) => read(s, "fFpct", "fpct") },
    ];
    if (isKidPitch) {
      defs.push(
        { label: "Ace (ERA)", kind: "dec2", hi: false, needsIp: true, get: (s) => read(s, "pEra", "era") },
        { label: "Best WHIP", kind: "dec2", hi: false, needsIp: true, get: (s) => read(s, "pWhip") }
      );
    }
    return defs
      .map((d) => {
        ...
        return best ? { label: d.label, player: best, value: fmt(bestVal, d.kind) } : null;
      })
  ...
  const attendance = useMemo(() => {
    const maps = [
      ...
      ...practices.filter((p: any) => p.attendance).map((p: any) => p.attendance),
    ];
    ...
    const perfect = Object.entries(byPlayer)
      .filter(([, c]) => c.marked >= Math.min(3, maps.length) && c.present === c.marked)
  ...
  const reportText = useMemo(() => {
    const lines = [`${team?.name || "Team"} — ${team?.currentSeason || ""} Season Report`.trim()];
    lines.push(
    ...
    if (leaders.length) {
      ...
      for (const l of leaders) lines.push(`• ${l.label}: ${l.player.name} (${l.value})`);
    }
    if (attendance?.teamRate != null) {
      lines.push("", `Team attendance: ${Math.round(attendance.teamRate * 100)}%`);
      if (attendance.perfect.length)
        lines.push(`Perfect attendance: ${attendance.perfect.map((p: any) => p.name).join(", ")}`);
    }
    if (improvers.length) {
      lines.push("", "Most improved (eval):");
      for (const im of improvers) lines.push(`• ${im.player.name} (+${im.delta.toFixed(1)})`);
    }
    return lines.join("\n");
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-3 gap-2">
          ...
          <div className="bg-surface-2 border border-line rounded-lg p-3 text-center">
            ...
            <div className="text-xl font-black tabular-nums text-ink mt-0.5">
              {summary.streakType ? `${summary.streakType}${summary.streakCount}` : "—"}
            </div>
          </div>
        </div>
          ...
          <div>
            ...
            <div className="flex items-baseline gap-2">
              ...
              <span className="t-meta text-ink-3">
                team · {attendance.events} event{attendance.events === 1 ? "" : "s"}
              </span>
            </div>
        ...
        {improvers.length > 0 && (
          <div>
            <div className="t-eyebrow text-ink-3 mb-1.5">Most Improved (Eval)</div>
