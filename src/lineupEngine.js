...
function buildPlayerProfile(p, grades) {
  ...
  // Use effective (blended) stats: blends current with last 1-2 past seasons,
...
// (math floor) and tally each player's "extra sits" = bench count - minimum.
...
function buildExtraSitHistory(games, currentGameId) {
  ...
  for (const g of games) {
    ...
    for (const [pid, count] of benchCount) {
      const cur =
        out.get(pid) ||
        { extraSits: 0, benchInn: 0, defInn: 0, expectedDef: 0 };
...
export function generateBattingOrder(profiledPlayers, battingSize, opts = {}) {
  ...
  if (capped) {
    ...
    if (count > 1) place(1, takeBest("overallScore"), "#2 Premium", "Best overall — modern #2 spot is a premium RBI position");
...
export function generateLineup(input) {
  ...
  battingLineup.forEach((player) => {
    ...
    if (eff.__blended && eff.__blendWeights?.current < 0.95) {
      player.battingReason.blendNote = `Stats blended (current ${Math.round(eff.__blendWeights.current * 100)}% / past ${Math.round((eff.__blendWeights.past1 + eff.__blendWeights.past2) * 100)}%)`;
    }
...
/* ----------------------------------------------------------------------------
   ...
   Inputs:
     ...
     numToBench            — bench slots per inning (= profiled.length - defenseSize)
...
function precomputeBenchSchedule(opts) {
  ...
  //   - The "extras" needed = totalBenchSlots - N * minSits.
  ...
  if (hasDisparity) {
    ...
    while (dIdx < donors.length && rIdx < recipients.length) {
      ...
      // the new max (donor + 1) and the new min (recipient - 1, or any
      ...
      recipient.delta += 1;
      if (
        dIdx + 1 < donors.length &&
        donor.delta < donors[dIdx + 1].delta
      )
        dIdx++;
      if (
  ...
  if (catcherInningPairs && catcherInningPairs.length > 0) {
    ...
    for (let i = 0; i < catcherInningPairs.length; i++) {
      ...
      // Find a catcher not yet used who can fit (target sits allows it)
      const candidate = eligibleC.find(({ p }) => !usedCatchers.has(p.id));
      if (!candidate) {
        return null; // infeasible (shouldn't happen with normal rosters)
      }
...
function tryBuildLineup(ctx) {
  ...
  // This-game spread penalty. The bench-count spread (max - min) should be
