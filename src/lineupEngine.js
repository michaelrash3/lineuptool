...
function precomputeBenchSchedule(opts) {
  ...
  // 8U and below: machine-pitch ages don't have real catcher mechanics
  // (no called strikes, no passed balls). A kid you've designated with a
  // primary infield position should anchor there rather than be drafted
  // behind the plate by the catcher pre-pin's tiebreaker. Older ages keep
  // the existing tier-fallback (coaches at 9U+ use explicit C restrictions
  // to gate the pool per their established workflow). Implemented as a
  // hard exclusion at the eligible-pool level; the tier-fallback below
  // still applies among the remaining (developmental-age-filtered) pool.
  const teamAgeNum = (() => {
  ...
  if (catcherInningPairs && catcherInningPairs.length > 0) {
    ...
    // budget to cover both innings of a catcher pair. At 8U-and-below,
    // also exclude kids with a non-C primaryPosition set.
