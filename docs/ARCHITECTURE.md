...
- **`advanceSeason` drops every sub** (`useTeamLifecycle`): the tournaments they were attached to are wiped by the rollover, so a carried-over sub would be an unreachable row.

Subs are added and removed from their tournament's detail page (`TournamentSubsPanel` → `/schedule/tournaments/:tournamentId/subs/new`), never from the Roster tab. Removing a sub still attached elsewhere just detaches them; the last detachment runs the full `removePlayer` cascade so they leave the lineups, attendance and pitch plans they were written into.
