...
| `players`                                                        | object[]                           | Roster — each entry carries `stats`, `pitching`, `comfortablePositions`, `restrictions`, `playerStatus` (no photo; see Images below). Also holds tournament subs, flagged `isSub` (see Tournament subs below)                                                                                                      |
...
### Tournament subs (guest players)

A coach who picks up a guest player for one tournament weekend should not have to put them on the roster. Subs are therefore stored **in the same `players` array** as everyone else, flagged with two fields (`src/types.ts`):

| Field              | Type     | Purpose                                                       |
| ------------------ | -------- | ------------------------------------------------------------- |
| `isSub`            | boolean  | Marks the entry a guest. Absent/false on every roster player. |
| `subTournamentIds` | string[] | Ids of the `tournaments` entries this sub was brought in for. |

One array rather than two, because the alternative means teaching the lineup engine, the lineup grid, the in-game swap model, the pitch-count math and every PDF generator about a second player source. Instead the rule is inverted, and lives in `src/utils/subPlayers.ts`:

- **Game-day surfaces ask `playsGame(player, gameId, tournaments)`** — true for every roster player, and for a sub only when `gameId` is in one of their tournaments' `gameIds`. Applied in `UIProvider`'s attendance defaults, `useLineupActions` (both engine pools), the Game Day Attendance grid, the Starting Pitcher picker, the in-game relief pool, and the What-If sandbox. It is deliberately _not_ attendance-based: an attendance map carried over from another game must not let a sub drift onto the rest of the schedule.
- **Season-long surfaces ask `isRosterPlayer(player)`** — false for subs and for departed players alike. This replaced the bare `!isDepartedPlayer(p)` test at every season-long call site, so behavior for teams with no subs is unchanged. Roster tab and cap, roster integrity, stats, evaluations, development, practices, availability, Player Info, team fees, tryout projections, season report, awards, roster CSV/PDF exports.
- **A sub's stats are never discarded.** Their stats and pitching log live on the player record, so a returning guest is ADDED to the next tournament (`addSubToTournament`) rather than re-created — a second row for the same kid would show the rest rules a fresh arm and under-count a real weekend workload. `advanceSeason` carries subs forward too, archiving their season line to `pastSeasons` exactly like a roster player; the rollover wipes `tournaments`, which leaves the attachment stale and makes them inert for games (`playsGame` resolves nothing) while keeping the record. They stay findable under **Tournament Subs** on the Roster tab, outside the roster list, the Active count and the cap. The only path that deletes a sub is the explicit, confirmed, undoable "Remove sub" on their last tournament.

Subs are managed from their tournament's detail page (`TournamentSubsPanel`): "New sub" mints one at `/schedule/tournaments/:tournamentId/subs/new`, "Add past sub" reuses a guest the team has had before. The Roster tab lists them read-only under Tournament Subs and links through to their profiles. Removing a sub still attached elsewhere just detaches them; the last detachment runs the full `removePlayer` cascade so they leave the lineups, attendance and pitch plans they were written into — and the confirm says plainly that their record and stats go with it.
...
- **Array writes are concurrency-safe.** Mutations to the team arrays — `players`, `practices`, `tournaments`, `tryoutSessions`, and (as ops) `games` — go through `updateTeamArrays` (op-based: append / removeById / mapEntries resolved from the latest committed state; see `src/utils/teamArrayUpdates.ts`), and finance mutations through the analogous `updateFinances` (`src/utils/financeUpdates.ts`). Two coaches editing near-simultaneously can no longer erase each other's changes. `games` ops are additionally TRANSLATED at the provider (Phase 3a of `docs/firestore-data-migration.md`): the computed next-array is diffed against the assembled union (`src/utils/teamEntityDocs.ts`) into full-entry per-doc `set()`s / `delete()`s on the games subcollection, riding ONE `writeBatch` with the rest of the op list's team-doc payload — cascades like remove-player → strip-from-games stay atomic. Both paths preserve `persistTeam`'s gates (game slimming, `photoUrl` strip, roster-wipe guard) and its optimistic apply + guarded revert. The portal-written families are likewise no longer team arrays — tryout signups, interest leads, player-info and availability submissions each live as per-entry docs in their own subcollection (`src/utils/tryoutSignupDocs.ts`, Phases 1 + 1b), and eval rounds persist per-doc to the `evalRounds` subcollection via `saveEvalRound` / `deleteEvalRound` (`src/utils/evalRounds.ts`, used by `src/hooks/useEvaluationCrud.ts`) — inherently concurrency-safe.
...
- **Service worker / PWA.** One worker, one registration. `vite-plugin-pwa` runs in `injectManifest` mode: it builds `src/service-worker.js` into `dist/service-worker.js` and bakes in the precache manifest (every hashed JS/CSS/HTML/PNG/SVG/woff2 the build emitted), but `injectRegister: null` stops it from also injecting a `registerSW.js` — registration happens only in `src/utils/serviceWorker.ts`, called from `src/index.tsx`. The worker serves navigations **network first** (a coach with signal always gets the newest deploy; the precached `index.html` is the offline fallback), precached assets cache-first, and other same-origin GETs stale-while-revalidate; cross-origin, `/api/*` and `/__/*` bypass it. It also owns the `notificationclick` handler behind game-day reminders. Firestore still needs network for live data — the SW covers the app shell only, not data offline. `manifest.json` also enables Add to Home Screen.
